...
def calculate_accuracy_metrics(plan, targets, meal_split):
	...
	if not plan or not plan.get("total_macros") or not plan.get("meals"):
		return {
			'calorie_accuracy': 0.0, 'macro_accuracy': 0.0, 'meal_distribution_accuracy': 0.0,
			'overall_accuracy': 0.0, 'calorie_deviation_pct': 1.0,
	...
	overall_accuracy = (calorie_accuracy * 0.4 + macro_accuracy * 0.4 + 0.2 * 0.8)  # Simplified
	
	return {
		'calorie_accuracy': calorie_accuracy,
		'macro_accuracy': macro_accuracy,
		'meal_distribution_accuracy': 0.8,
		'overall_accuracy': overall_accuracy,
		'calorie_deviation_pct': calorie_deviation_pct,
		'macro_deviations': {'Protein': 0.1, 'Carbs': 0.1, 'Fat': 0.1},
		'meal_distribution_deviations': {'breakfast': 0.1, 'lunch': 0.1, 'dinner': 0.1}
	}
...
def generate_brute_force_plans(bfast_combos, lunch_combos, dinner_combos, targets, meal_split, macro_priority, max_combinations=10000):
	...
	metrics = {
		'algorithm': 'brute_force', 'execution_time': execution_time,
		'combinations_evaluated': combinations_evaluated, 'total_possible': total_possible,
...
def generate_genetic_algorithm_plans_with_metrics(bfast_combos, lunch_combos, dinner_combos, targets, meal_split, macro_priority, population_size=100, generations=50, top_n=100):
	import time
	start_time = time.time()
	metrics = {'algorithm': 'genetic', 'execution_time': 0, 'fitness_evaluations': 0, 'population_size': population_size, 'generations': generations}
...
def compare_algorithms(bfast_combos, lunch_combos, dinner_combos, targets, meal_split, macro_priority):
	...
	print(f"\nDetailed Metrics:")
	print(f"Brute Force: {brute_metrics['execution_time']:.3f}s, Score: {brute_metrics['best_score']:.2f}")
	print(f"Genetic: {genetic_metrics['execution_time']:.3f}s, Score: {genetic_metrics['best_score']:.2f}")
...
def compare_algorithms_route():
	if request.method == "GET":
		return render_template("mealplan.html", plan=None, targets=None, user_inputs=None, plan_index=0, total_plans=0, breakfast_pct=25, lunch_pct=40, dinner_pct=35, macro_focus="protein", current_date=datetime.now().strftime("%Y-%m-%d"), error_message=None)
	...
	try:
		...
		return render_template("algorithm_comparison.html", targets=targets, user_inputs=user_inputs, comparison_metrics=comparison_metrics, best_brute=best_brute, best_genetic=best_genetic, meal_split=meal_split, macro_focus=macro_choice)
							
	except Exception as e:
		error_message = str(e)
		return render_template("mealplan.html", plan=None, targets=None, user_inputs=None, plan_index=0, total_plans=0, breakfast_pct=25, lunch_pct=40, dinner_pct=35, macro_focus="protein", current_date=datetime.now().strftime("%Y-%m-%d"), error_message=error_message)
